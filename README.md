...
核心库只定义存储契约与调用时机，不实现数据库或业务状态规则。宿主可选择注册以下接口；未注册时保持原有执行流程：

- `ConversationStateStore` 与 `ConversationStatePolicy`：策略指定分类前和单意图阶段要读取的状态键，内核按 `conversationId` 读取最新值并加入受控上下文。适合当前控制模式等会影响短指令理解的会话状态。
- `SessionMemoryStore` 与 `SessionMemoryRecallPolicy`：策略指定分类前或单意图阶段需要的精确键或检索文本。只有策略返回查询时才调用宿主存储。适合会话内明确保存的事实；业务能力在执行成功后调用存储接口写入或删除。
- `ExecutionCheckpointStore`：按会话 ID 与执行 ID 保存内核生成的 JSON 快照。相同执行 ID 再次调用时恢复计划与已完成结果；中断时仍在运行的节点转为等待确认，不自动重放可能已有外部副作用的工具。

状态和记忆均严格限定在同一 `conversationId`。这些接口可由内存、数据库或其他宿主实现；需要跨服务重启恢复时，宿主须提供持久化实现，并保留相应会话与执行 ID。
